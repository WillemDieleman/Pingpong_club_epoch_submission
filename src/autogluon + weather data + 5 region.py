...
# ---------------------------------------------------------------
# Community code → 5 climate groups mapping
# ---------------------------------------------------------------
...
# PySpark mapping expression
_map_expr = F.create_map([F.lit(x) for x in chain(*CC_TO_GROUP.items())])

# ---------------------------------------------------------------
# Aggregate raw rows → one row per 15-min interval PER GROUP
# item_id = climate group (5 groups)
# ---------------------------------------------------------------
agg_df = (
    df
...
print(f"Groups: {sorted(ts_pdf['item_id'].unique())}")
...
# ---------------------------------------------------------------
# Load weather forecasts, regional holidays,
# merge into ts_pdf as known covariates for AutoGluon
# (aggregated to 5 climate groups)
# ---------------------------------------------------------------

# ========================  WEATHER  ========================
# Map admin_name → community_code → climate group
ADMIN_TO_CC = {
...
weather_pdf["group"] = weather_pdf["admin_name"].map(ADMIN_TO_GROUP)
weather_pdf = weather_pdf.dropna(subset=["group"])
weather_pdf["date"] = pd.to_datetime(weather_pdf["date"])

weather_agg = (
    weather_pdf
    .groupby(["group", "date"])[WEATHER_RAW_COLS]
    .mean()
    .reset_index()
...
print(f"Weather merged: {WEATHER_COLS}")
...
holidays_pdf["group"] = holidays_pdf["community_code"].map(CC_TO_GROUP)
...
print(f"Holidays merged: {HOLIDAY_COLS}")
...
print(f"\n{'='*60}")
print(f"Known covariate columns ({len(KNOWN_COV_COLS)}): {KNOWN_COV_COLS}")
print(f"ts_pdf shape: {ts_pdf.shape}")
print(f"Missing weather rows:  {n_weather_nulls:,} / {len(ts_pdf):,} ({100*n_weather_nulls/len(ts_pdf):.1f}%)")
...
#   - active_kw:          predictions are already total kW per region
#   - avg_kw_per_client:  predictions × n_clients → total kW per region
# Then sum across regions and compute MAE on the grid total.
# ---------------------------------------------------------------
...
if target == "avg_kw_per_client":
    # Scale predictions back to total kW using actual n_clients
    pred_with_clients = pred_reset.merge(
        actual_reset[["item_id", "timestamp", "n_clients"]],
        on=["item_id", "timestamp"],
        how="left",
    )
    pred_with_clients["total_kw_pred"]   = pred_with_clients["mean"] * pred_with_clients["n_clients"]
    actual_reset["total_kw_actual"] = actual_reset[target] * actual_reset["n_clients"]

    pred_total   = pred_with_clients.groupby("timestamp")["total_kw_pred"].sum()
    actual_total = actual_reset.groupby("timestamp")["total_kw_actual"].sum()
    scaling_note = f"(scaled by n_clients per region)"
else:
    # target is active_kw — already in total kW
    pred_total   = pred_reset.groupby("timestamp")["mean"].sum()
    actual_total = actual_reset.groupby("timestamp")[target].sum()
    scaling_note = "(direct sum)"
...
print(f"Known covariates:                   {known_cov_names}")
print(f"Best model avg per-region MAE:      {abs(best_row['score_val']):,.2f} (in {target} units)")
print(f"Sum of regional MAEs (upper bound): {abs(best_row['score_val']) * n_items:,.0f}")
print(f"Competition-equivalent total MAE:   {total_MAE:,.0f} kW {scaling_note}")
print(f"{'='*55}")
...
# Works for BOTH targets:
#   - active_kw:          sum predictions directly
#   - avg_kw_per_client:  predictions × n_clients → total kW
# ---------------------------------------------------------------
import pandas as pd
...
print(f"Target: {target}")
scaling_note = "(scaled by n_clients)" if target == "avg_kw_per_client" else "(direct sum)"
print(f"Scaling: {scaling_note}\n")
...
for i, day in enumerate(test_days):
    ...
    # Flatten
    pred_reset = preds.reset_index()
    actual_reset = actual_ts.reset_index()
    ...
    if target == "avg_kw_per_client":
        ...
        pred_total = pred_with_clients.groupby("timestamp").apply(
            lambda g: (g["mean"] * g["n_clients"]).sum()
        )
        actual_total = actual_reset.groupby("timestamp").apply(
            lambda g: (g[target] * g["n_clients"]).sum()
        )
    else:
        pred_total = pred_reset.groupby("timestamp")["mean"].sum()
        actual_total = actual_reset.groupby("timestamp")[target].sum()
...
print(f"Target: {target} {scaling_note}")
print(f"{'='*55}")
