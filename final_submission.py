...
class EnergyConsumptionModel:
    ...
    with weather and holiday covariates, per-region time series.
    ...
    def _load_weather_15min():
        """Load weather forecasts, aggregate per region, resample to 15-min."""
        ...
        weather_pdf["community_code"] = weather_pdf["admin_name"].map(
            EnergyConsumptionModel.ADMIN_TO_CC
        )
        ...
        weather_agg = (
            weather_pdf
            .groupby(["community_code", "date"])[EnergyConsumptionModel.WEATHER_RAW_COLS]
            .mean()
            .reset_index()
    ...
    def _load_holidays():
        ...
        return holidays_pdf[["date", "community_code", "is_holiday", "is_bridge"]]

    @staticmethod
    def convert_train_data(df):
        """Aggregate PySpark DF to 15-min per-region energy, merge weather + holidays."""
        # Aggregate per region per 15-min interval
        agg_df = (
            df
        ...
        # Merge holidays
        holidays_pdf = EnergyConsumptionModel._load_holidays()
